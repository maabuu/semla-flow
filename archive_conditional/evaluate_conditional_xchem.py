...
def evaluate(mol_pred: Mol, frags: Mol) -> dict[str, float | int | str]:
    ...
    df["esp_sim"] = df["fragment"].apply(lambda f: compute_esp_sim(mol_probe=mol_pred, mol_ref=f))
    df["shape_sim"] = df["fragment"].apply(lambda f: compute_shape_tanimoto(mol_probe=mol_pred, mol_ref=f))
    ...
    df["sucos"] = df["fragment"].apply(lambda f: compute_sucos(mol_probe=mol_pred, mol_ref=f))
    df["feature_map_score"] = df["fragment"].apply(lambda f: compute_feature_map_score(mol_probe=mol_pred, mol_ref=f))

    summary = df[["sucos", "esp_sim", "shape_sim", "feature_map_score"]].describe().T.reset_index()
