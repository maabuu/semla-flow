...
if __name__ == "__main__":
    with open(r'C:\Users\ziv\Documents\FlowMol\shephered\NP_analogues_2500\shepherd_samples_NP_1.pickle', 'rb') as f:
    ...
    filtered = [
        ...
        if int(ref_size*0.6) <= len(s['x1']['atoms']) <= ref_size
    ]

    rdkit_mols = []
    for generated_sample in filtered:
        rdkit_mol, _, _ = get_mol_from_atom_pos(atoms= generated_sample['x1']['atoms'],
            positions=generated_sample['x1']['positions']
        ) # rdkit_mol will be None if it fails our rdkit validity checks which is quite strict due to explicit hydrogens
        rdkit_mols.append(rdkit_mol)

    writer = Chem.SDWriter(r'C:\Users\ziv\Documents\FlowMol\shepered\NP_analogues_2500\all_mols_1.sdf')
