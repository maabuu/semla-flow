...
from utils.docking import get_random_id, BaseDockingTask
...
class VinaDock(object):
    ...
    def dock(self, score_func='vina', seed=0, mode='dock', exhaustiveness=8,
             save_pose=False,save_path = None):  # seed=0 mean random seed
        v = Vina(sf_name=score_func, seed=seed, verbosity=0)
        v.set_receptor(self.prot_pdbqt)
        print('set_receptor')
        v.set_ligand_from_string(self.lig_pdbqt)
        print('set_ligand_from_string')
        ...
        if not save_pose:
            ...
            if mode == 'score_only':
                ...
                with open(tmp.name, 'w') as f:
                    v.write_pose(save_path, overwrite=True)
...
class VinaDockingTask(BaseDockingTask):
    ...
    def run(self, mode='dock', exhaustiveness=8 , save_path = None):
        ...
        lig = meeko_prep.prepare(self.ligand_rdmol)
        ligand_pdbqt = meeko_prep.write_pdbqt_string(lig)
        ...
        score, pose = dock.dock(score_func='vina', mode=mode, exhaustiveness=exhaustiveness, save_pose=True, save_path = save_path)
